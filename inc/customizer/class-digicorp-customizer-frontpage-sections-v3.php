<?php
/**
* Class to create front page sections - Version 3
 * in customizer
 */
class Digicorp_Customizer_Frontpage_Section_V3
{
  ...
  function digicorp_customize_register( $wp_customize )
  {
    ...
    // CTA Text
    $wp_customize->add_setting( $this->prefix_section . '_cta_button_text', array(
    	'sanitize_callback' => 'wp_kses_post',
    	'default'           => '',
    	'transport'         => 'postMessage',
    ) );
    $wp_customize->add_control(  new WP_Customize_Control( $wp_customize, $this->prefix_section . '_cta_button_text', array(
    ...
    // CTA link
    $wp_customize->add_setting( $this->prefix_section . '_cta_button_link', array(
    	'sanitize_callback' => 'wp_kses_post',
    	'default'           => '',
    	'transport'         => 'postMessage',
    ) );
    $wp_customize->add_control(  new WP_Customize_Control( $wp_customize, $this->prefix_section . '_cta_button_link', array(
    	'label'       => __( 'Call To Action Link', 'digicorpdomain' ),
    	'type' 				=> 'text',
    	'section'     => $this->prefix_section,
    ) ) );

    // CTA page
    $wp_customize->add_setting( $this->prefix_section . '_cta_button_page', array(
    	'sanitize_callback' => 'wp_kses_post',
    	'default'           => '0',
    	'transport'         => 'postMessage',
    ) );
    $wp_customize->add_control(  new WP_Customize_Control( $wp_customize, $this->prefix_section . '_cta_button_page', array(
    ...
    ?>
      ...
      ( function( $ ) {
          ...
          $(document).ready( function() {
            ...
            wp.customize( '<?php echo $this->prefix_section ?>_cta_button_text', function( value ) {
              value.bind( function( to ) {
                if ( '' !== to.trim() ) {
                  $( '<?php echo $this->section_selector ?> .section_cta_container' ).removeClass( 'customizer-display-none' );
                } else {
                  $( '<?php echo $this->section_selector ?> .section_cta_container' ).addClass( 'customizer-display-none' );
                }
                $( '<?php echo $this->section_selector ?> .section_cta_text' ).html( to );
              } );
            } );

            wp.customize( '<?php echo $this->prefix_section ?>_cta_button_link', function( value ) {
              value.bind( function( to ) {
                $( '<?php echo $this->section_selector ?> .section-cta-button' ).attr( 'href', to );
              } );
            } );
